 ...
 * @author Luke
 */
public class TFFirTree extends TetrisFigure {

    //boolean[][] gameAr;
    int position;

    public TFFirTree(Graphics graphics, Color paintColor, int size, boolean[][] gameAr) {
        ...
        if (position == 1) { //1
            ...
            unitsArray[3] = new Unit(this.graphics, 5 * size, -3 * size, size, this.paintColor, gameAr);
        }
        if (position == 2) { //2
        ...
        if (position == 3) { //3
            ...
            unitsArray[2] = new Unit(this.graphics, 6 * size, -3 * size, size, this.paintColor, gameAr);
            unitsArray[3] = new Unit(this.graphics, 5 * size, -2 * size, size, this.paintColor, gameAr);
        }
        if (position == 4) { //4
            unitsArray[0] = new Unit(this.graphics, 5 * size, -4 * size, size, this.paintColor, gameAr);
            unitsArray[1] = new Unit(this.graphics, 5 * size, -3 * size, size, this.paintColor, gameAr);
            unitsArray[2] = new Unit(this.graphics, 5 * size, -2 * size, size, this.paintColor, gameAr);
            unitsArray[3] = new Unit(this.graphics, 4 * size, -3 * size, size, this.paintColor, gameAr);
        }
    }

    @Override
    public void rotateTetrisFigure(int width, int height) {
        //System.out.println("Вращение ёлки пока ещё не поддерживается");
        if (isStoped) {
            return;
        }
        ...
        if (position == 1) {
            if (rotLine == gameAr.length - 1) {
                System.out.println("gameAr.length - 1 = " + (gameAr.length - 1));
