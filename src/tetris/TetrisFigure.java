...
public abstract class TetrisFigure {
    ...
    public boolean canMoveDown(int height) {
        //Тут буду проверять можно ли начинать двигать фигуру
        for (int i = 0; i < unitsArray.length; i++) {
            if (unitsArray[0].getLine() < 0
                    && unitsArray[1].getLine() < 0
                    && unitsArray[2].getLine() < 0
                    && unitsArray[3].getLine() < 0
                    && gameAr[0][unitsArray[i].getColumn()] == true) {                
                return false;
            }
        }
